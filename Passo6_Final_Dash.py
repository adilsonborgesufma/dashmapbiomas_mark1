...
try:
    ee.Initialize(project='ee-adilsonborges')  # Tenta inicializar com o projeto existente
except Exception as e:
    try:
        ee.Authenticate()  # Se falhar, tenta autenticar
        ee.Initialize(project='ee-adilsonborges')  # E inicializa novamente
    except:
...
def reclassify_bands(image, codes, new_classes):
    ...
    for year in range(1985, 2024):  # Itera de 1985 a 2023, tem que colocar 1 ano a mais para pegar o último ano
...
########################### OUTRAS OPÇÕES DE DEFINIÇÃO DE ÁREA ###############################################
...
#################################### Visualização no mapa - Colorido automaticamente ############################
...
######################## Layers - Adiciona camadas para cada ano selecionado ####################################
