...
df[cols_numericas] = df[cols_numericas].replace(',', '.', regex=True).astype(float)    # converte vírgula para pontos

# soma todos os valores e faz a média, transforma os valores e insere em uma coluna
...
# Adiciona uma coluna para uma análise cruzada entre eles
...
#Calcula a matriz de correlação em duas matrizes
...
# Ajustar as margens dos gráficos
...
#Função para plotar caso não venha nenhum plot
